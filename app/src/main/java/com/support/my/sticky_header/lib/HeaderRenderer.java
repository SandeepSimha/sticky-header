...
public class HeaderRenderer {
    ...
    /**
     * Draws a Stacked View to a canvas, offsetting by some x and y amount and with a rotate factor.
     *
     * @param canvas      the canvas on which to draw the header
     * @param stackedView to draw
     * @param offset      to extract x and y offsets
     * @param rotation    factor
     */
    public void drawStack(Canvas canvas, View stackedView, Rect offset, float rotation) {
        canvas.save();
        canvas.translate(offset.left, offset.top);
        canvas.rotate(rotation, stackedView.getWidth() / 2, stackedView.getHeight() / 2);
        stackedView.draw(canvas);
        canvas.restore();
    }
